...
import { ComboType, COMBO_TYPE_LABELS, toCents, fromCents } from '@/entities/combo/model/types';
...
export const EditComboPage = () => {
  ...
  const { currentCombo, isLoading, fetchComboById, updateCombo } = useCombosStore();
  ...
  }, [fetchComboById, fetchProducts, comboId, restaurantId]);

  useEffect(() => {
    if (currentCombo) {
      setFormData({
        nombre: currentCombo.nombre,
        tipo_combo_id: currentCombo.tipo_combo_id,
        descripcion: currentCombo.descripcion || '',
        precio: fromCents(currentCombo.precio).toFixed(2),
        productos: currentCombo.productos?.map((p) => ({
    ...
    <div className="p-6 max-w-2xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-3xl font-semibold text-primary">Editar Combo</h1>
        </div>
      </div>
